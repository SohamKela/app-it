...
if command -v rg >/dev/null 2>&1; then
    ARTIFACT_URLS="$(rg --no-heading -n -E 'https://claude\.ai/[^ )"'\''>]+' \
        -g '*.md' -g '*.json' -g '*.html' -g '*.js' -g '*.jsx' -g '*.ts' -g '*.tsx' \
        -g '!node_modules/**' -g '!desktop/**' -g '!assets/icons/**' -g '!.git/**' . 2>/dev/null | head -8 || true)"
    ARTIFACT_API="$(rg --no-heading -n -E 'window\.claude|window\.storage|claude\.complete|claude\.request' \
        -g '*.html' -g '*.js' -g '*.jsx' -g '*.ts' -g '*.tsx' \
        -g '!node_modules/**' -g '!desktop/**' -g '!assets/icons/**' -g '!.git/**' . 2>/dev/null | head -8 || true)"
else
    ARTIFACT_URLS="$(grep -RnE 'https://claude\.ai/[^ )"'\''>]+' . \
    ...
    ARTIFACT_API="$(grep -RnE 'window\.claude|window\.storage|claude\.complete|claude\.request' . \
